"""OSINT proxy — ported from Potencjal backend/app/services/osint.py.

Fetches data from Polish public registries:
  - VAT White List (free)
  - eKRS (free)
  - CEIDG (requires API key)
  - GUS REGON (requires API key)
"""
...
async def fetch_vat_whitelist(nip: str) -> OsintResult:
    ...
    return OsintResult(
        ...
        vat_status=vat_status, regon=regon or None, krs=krs or None, raw=data,
    )
...
async def fetch_ekrs(nip: str) -> OsintResult:
    ...
    return OsintResult(
        ...
        years_active=years_active, krs=krs or None, regon=regon or None, raw=data,
    )
...
async def fetch_gus(nip: str) -> OsintResult:
    ...
    return OsintResult(
        ...
        regon=regon or None, pkd=pkd or None, raw={"response_snippet": search_resp.text[:2000]},
    )
...
async def enrich_lead(nip: str) -> tuple[list[OsintResult], dict]:
    """Fetch all OSINT sources and merge using first-non-null strategy."""
